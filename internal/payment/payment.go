...
func calculateExpireAt(currentExpireAt *time.Time, daysToAdd int) time.Time {
	now := time.Now()
	if currentExpireAt != nil && currentExpireAt.After(now) {
		return currentExpireAt.AddDate(0, 0, daysToAdd)
	}
	return now.AddDate(0, 0, daysToAdd)
}

func (s PaymentService) generateInviteLink(ctx context.Context) (string, error) {
	inviteLink, err := s.telegramBot.CreateChatInviteLink(ctx, &bot.CreateChatInviteLinkParams{
...
func (s PaymentService) ProcessPurchaseById(ctx context.Context, purchaseId int64) error {
	...
	inviteUrl, err := s.generateInviteLink(ctx)
	if err != nil {
		slog.Error("Failed to generate invite link", "error", err)
		return err
	}

	expireAt := calculateExpireAt(customer.ExpireAt, purchase.Month*config.DaysInMonth())
	...
	if purchase.InvoiceType == database.InvoiceTypeYookasa && s.moynalogClient != nil {
		go func() {
			...
			if err := s.sendReceiptToMoynalog(moynalogCtx, purchase); err != nil {
				...
				_, sendErr := s.telegramBot.SendMessage(context.Background(), &bot.SendMessageParams{
...
func (s PaymentService) CancelTributePurchase(ctx context.Context, telegramId int64) error {
	...
	// Just decrease the expireAt locally
	expireAt := calculateExpireAt(customer.ExpireAt, -tributePurchase.Month*config.DaysInMonth())
...
func (s PaymentService) createTelegramInvoice(ctx context.Context, amount float64, months int, customer *database.Customer) (url string, purchaseId int64, err error) {
	...
	invoiceUrl, err := s.telegramBot.CreateInvoiceLink(ctx, &bot.CreateInvoiceLinkParams{
		...
		Payload:     fmt.Sprintf("%d", purchaseId),
	})
