...
func main() {
	...
	paymentService := payment.NewPaymentService(tm, purchaseRepository, customerRepository, b, cryptoPayClient, yookasaClient, plategaClient, cache, moynalogClient)
	...
	h := handler.NewHandler(paymentService, tm, customerRepository, purchaseRepository, cryptoPayClient, yookasaClient, cache, menuService)
	...
	mux.Handle("/healthcheck", fullHealthHandler(pool))
...
func fullHealthHandler(pool *pgxpool.Pool) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"%s","db":"%s","time":"%s","version":"%s","commit":"%s","buildDate":"%s"}`,
			status["status"], status["db"], status["time"], Version, Commit, BuildDate)
	})
}
...
func checkCryptoPayInvoice(
	...
	for _, invoice := range *invoices {
		if invoice.InvoiceID != nil && invoice.IsPaid() {
			if !strings.HasPrefix(invoice.Payload, "purchaseId=") {
				continue
			}
			
			purchaseIDStr := strings.TrimPrefix(invoice.Payload, "purchaseId=")
			purchaseID, err := strconv.Atoi(purchaseIDStr)
			if err != nil {
				slog.Error("Error parsing purchase id from crypto invoice", "error", err, "payload", invoice.Payload)
				continue
			}
			
			err = paymentService.ProcessPurchaseById(ctx, int64(purchaseID))
			if err != nil {
